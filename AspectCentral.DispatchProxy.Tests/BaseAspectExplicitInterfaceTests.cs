...
namespace AspectCentral.DispatchProxy.Tests
{
    ...
    public class BaseAspectExplicitInterfaceTests
    {
        ...
        public class ExplicitImpl : IExplicit
        {
            ...
            string IExplicit.Hidden() => "hidden";
        }
        ...
        private static BaseAspectTestClass<IExplicit> CreateAspect()
        {
            ...
            var config = new AspectConfiguration(new ServiceDescriptor(typeof(IExplicit), typeof(ExplicitImpl), ServiceLifetime.Transient));
        ...
        {
            var aspect = CreateAspect();
            var target = typeof(IExplicit).GetMethod(nameof(IExplicit.Add), new[] { typeof(int), typeof(int), typeof(int) })!;
