...
public class LoggingAspect<T> : BaseAspect<T> where T : class?
{
    ...
    /// <param name="type">
    /// The concrete implementation <see cref="System.Type" /> behind <paramref name="instance" />; used for
    /// the logger category and method-info lookup.
    /// </param>
    /// <param name="loggerFactory">
    /// The factory used to create the logger; the category is set to <c>type.FullName</c> so
    /// consumers can filter per implementation.
    /// </param>
    ...
    /// The <see cref="LoggingAspectFactory" /> type that owns this proxy. Recorded on
    /// every <see cref="System.Diagnostics.Activity" /> as <c>aspect.factory</c>.
    /// </param>
    ...
    public static T Create(T instance, Type type, ILoggerFactory loggerFactory,
        IAspectConfigurationProvider aspectConfigurationProvider, Type loggingAspectFactoryType)
    {
    ...
    /// <summary>
    /// Logs the intercepted method's return value, when present, and emits the completion event.
    /// </summary>
    /// <param name="aspectContext">
    /// The invocation context containing the resolved invocation string and return value.
    /// </param>
    ...
    /// <summary>
    /// Logs the start of an intercepted method invocation.
    /// </summary>
    /// <param name="aspectContext">
    /// The invocation context containing the resolved invocation string.
    /// </param>
