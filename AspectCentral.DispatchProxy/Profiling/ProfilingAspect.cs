...
public class ProfilingAspect<T> : BaseAspect<T> where T : class?
{
    ...
    /// The <see cref="ProfilingAspectFactory" /> type that owns this proxy. Recorded
    /// on every <see cref="System.Diagnostics.Activity" /> as <c>aspect.factory</c>.
    /// </param>
    ...
    public static T Create(T instance, Type type, ILoggerFactory loggerFactory,
        IAspectConfigurationProvider aspectConfigurationProvider, Type profilingAspectFactoryType)
    {
    ...
    /// <summary>
    /// Stops the per-invocation stopwatch and logs the elapsed duration.
    /// </summary>
    ...
    /// <summary>
    /// Starts timing an intercepted method invocation.
    /// </summary>
    /// <param name="aspectContext">
    /// The invocation context for the method call about to be invoked.
    /// </param>
